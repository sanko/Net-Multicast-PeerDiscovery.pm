...
This implementation is B<RFC 6724 compliant> and supports both IPv4 and IPv6 multicast:
...
It correctly handles B<IPv6 Link-Local> addresses (C<fe80::/10>), preserving the C<scope_id> required for local
connectivity.
...
=head2 C<on( peer_found => sub ($info) { ... } )>
